...
def scan_known_people(known_people_folder):
    ...
    for file in image_file_in_folder(known_people_folder):
...
def test_image(image_to_check, known_names, known_face_encodings, tolerance=0.6, show_distance=False):
    ...
    # scale the image down so things gets faster
...
def process_images_in_process_pool(image_to_check, known_names, known_face_encodings, number_of_cpus, tolerance,
                                   show_distance):
    if number_of_cpus == -1:
        processes = None
    else:
